...
def get_parser():
    ...
    parser.add_argument(
            '--eval_dir', default='data', type=str, action='store')  # can keep, unused now if you want
    parser.add_argument(
    ...
    # NEW: txt files (by Linda)
    parser.add_argument(
            '--word_pairs_file', type=str, required=True,
            help='Path to txt file with word1<TAB>word2 per line')
    parser.add_argument(
            '--human_scores_file', type=str, required=True,
            help='Path to txt file with one human similarity score per line')
    parser.add_argument(
...
# function added by linda
def load_txt_sim_data(word_pairs_file, human_scores_file, dataset_name):
    """
    word_pairs_file: each line 'word1<TAB>word2'
    human_scores_file: each line a numeric similarity score corresponding to that pair
    """
    words_left = []
    words_right = []
    relatedness = []
    ...
    with open(word_pairs_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if len(parts) != 2:
                raise ValueError(
                    f"Expected 2 columns (word1<TAB>word2) but got {len(parts)} in line: {line}"
                )
            words_left.append(parts[0])
            words_right.append(parts[1])

    # Read human similarity scores
    with open(human_scores_file, 'r', encoding='utf-8') as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            relatedness.append(float(line))

    if not (len(words_left) == len(words_right) == len(relatedness)):
        raise ValueError(
            f"Length mismatch: {len(words_left)} word_left, "
            f"{len(words_right)} word_right, {len(relatedness)} scores"
        )

    # Single dataset called 'custom'
    string_lists_sims = {
        dataset_name: dict(
...
class WordSimRunner(use_lm_eval.LMEvalRunner):
    ...
    def load_string_lists_sims(self):
        """
        Load a single word similarity dataset from two txt files:
        - args.word_pairs_file: word1<TAB>word2 per line
        - args.human_scores_file: human similarity score per line
        """
        self.string_lists_sims = load_txt_sim_data(
            self.args.word_pairs_file,
            self.args.human_scores_file,
            self.args.dataset_name
        )
        # Just one dataset key: 'custom'
        self.datasets = list(self.string_lists_sims.keys())
...
python your_script.py \
  ...
  --output_csv per_dataset_max_layer.csv \
  --output_final_avg_score_csv final_avg.csv \
  --word_pairs_file /path/to/word_pairs.txt \
  --human_scores_file /path/to/human_scores.txt \
  --pretrained gpt2
'''