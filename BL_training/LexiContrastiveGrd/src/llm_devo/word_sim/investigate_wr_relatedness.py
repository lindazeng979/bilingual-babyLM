...
#BELOW RESULT_DIR CODE CHANGED BY STEVEN
...
def check_and_create_csv(file_name):
    ...
    if not os.path.isfile(file_name):
        ...
        with open(file_name, mode='w', newline='') as file:
            ...
            # Write the header
            writer.writerow(["Model", "Eval Dataset", "Max Layer Score"])
        print(f"File '{file_name}' created with default header.")
    else:
        print(f"File '{file_name}' already exists.")


def append_row_to_csv(file_name, model_path, input_str, best_layer_score):
    ...
    with open(file_name, mode='a', newline='') as file:
        writer = csv.writer(file)
        # Write the new row
        writer.writerow([model_path, input_str, best_layer_score])
    print(f"Appended row to '{file_name}'.")


def check_and_create_final_avg_csv(file_name):
    # Check if the file exists
    if not os.path.isfile(file_name):
        # If the file does not exist, create it with the default header
        with open(file_name, mode='w', newline='') as file:
            writer = csv.writer(file)
            # Write the header
            writer.writerow(["Model", "Final Avg Score"])
        print(f"File '{file_name}' created with default header.")
    else:
        print(f"File '{file_name}' already exists.")


def append_row_to_final_avg_csv(file_name, model_path, final_avg_score):
    # Format the final average score as a percentage with two decimal places
    final_avg_score_formatted = f"{final_avg_score:.2f}"
    ...
    with open(file_name, mode='a', newline='') as file:
        writer = csv.writer(file)
        # Write the new row
        writer.writerow([model_path, final_avg_score_formatted])
    print(f"Appended row to '{file_name}'.")
...
def get_parser():
    ...
    parser.add_argument(
            '--output_final_avg_score_csv', default=None, type=str, action='store')
    parser.add_argument(
...
class WordSimRunner(use_lm_eval.LMEvalRunner):
    ...
    def get_avg_token_and_embedding_counts(self, string_list, add_top_blank=True):
        """
        Returns:
        avg_tokens: average number of tokens produced by the tokenizer per input string
        avg_emb_tokens: average number of embedding vectors per input string at a chosen layer
                        (should match tokens unless you later drop BOS/EOS etc.)
        """
        ...
        for s in string_list:
            ...
            if self.args.pretrained == "gpt2":
                inp = tokenizer.bos_token + inp  # mirrors your existing logic

            enc = tokenizer([inp], return_tensors="pt", add_special_tokens=True)
            input_ids.append(enc.input_ids)
            token_lens.append(enc.input_ids.shape[1])
        ...
        # Get embeddings for all layers; pick one layer to count token positions.
        # new_returned_embeddings[layer] has shape (num_examples, hidden_dim)
        # BUT that's because you currently extract only ONE position: layer_vectors[:, self.extraction_id]
        #
        # If you want "embedding count per line" as "number of token vectors available",
        # you need to extract *all token positions*, not just one (see section 2 below).
        #
        # For now, embedding count per line in your CURRENT setup is always 1 per string,
        # because you collapse to one vector per string via extraction_id.
        #
        # So we return 1.0 unless you modify extraction to keep all positions.
        avg_emb_tokens = 1.0

        return avg_tokens, avg_emb_tokens
    ...
    def get_all_model_inputs(self, string_list):
        ...
        for string in string_list:
            ...
            inputs = tokenizer(
                    ...
                    add_special_tokens=True)            # tokenizes each left word and right word separaetly
    ...
    def get_all_embeddings(
        ...
        for _len in diff_lens:
            ...
            for sta_idx in range(0, len(now_idx), self.batch_size):
                ...
                #    representation = layer_vectors[:, self.extraction_id]   #Indexing with [:, self.extraction_id] selects one token position (e.g. last token or second-to-last), so representation becomes no seq length
                #    embeddings.append(representation)

                embeddings = []
                eos_id = getattr(self.lm.tokenizer, "eos_token_id", None)

                for layer_vectors in hidden_states:
                    # keep as torch for pooling
                    if isinstance(layer_vectors, np.ndarray):
                        layer_vectors = torch.from_numpy(layer_vectors)

                    device = layer_vectors.device  # whatever device the model outputs on

                    # layer_vectors: [B, T, H]
                    B, T, H = layer_vectors.shape

                    if len(attention_masks) > 0 and new_att_masks is not None:
                        # lengths = number of non-pad tokens
                        lengths = _att_masks.to(device).sum(dim=1).long()  # [B]
                    else:
                        lengths = torch.full((B,), T, device=layer_vectors.device, dtype=torch.long)

                    # Decide whether to exclude EOS per row
                    # last valid token index is lengths-1
                    last_idx = lengths - 1  # [B]
                    has_eos = torch.zeros(B, device=layer_vectors.device, dtype=torch.bool)
                    if eos_id is not None:
                        # check if the last token is EOS
                        last_tokens = _input_ids.to(layer_vectors.device)[torch.arange(B), last_idx]
                        has_eos = (last_tokens == eos_id)

                    # end = lengths-1 if EOS present else lengths
                    end = lengths - has_eos.long()  # [B] end is exclusive

                    # Build a mask for tokens [0, end)
                    ar = torch.arange(T, device=layer_vectors.device).unsqueeze(0)  # [1, T]
                    mask = ar < end.unsqueeze(1)  # [B, T]

                    # mean pool over masked tokens
                    masked = layer_vectors * mask.unsqueeze(-1)  # [B, T, H]
                    denom = mask.sum(dim=1).clamp(min=1).unsqueeze(-1)  # [B, 1]
                    representation = masked.sum(dim=1) / denom  # [B, H]

                    embeddings.append(representation.detach().cpu().numpy())
    ...
    def get_layerwise_scores_for_one_model_one_ds(
        ...
        all_layer_scores = [] #ADDED BY STEVEN

        avg_tok_left, avg_emb_left = self.get_avg_token_and_embedding_counts(words_left, add_top_blank=add_top_blank)
        avg_tok_right, avg_emb_right = self.get_avg_token_and_embedding_counts(words_right, add_top_blank=add_top_blank)

        with open(self.args.output_file, 'a') as file:
            file.write(f"Avg token count (left): {avg_tok_left:.2f}\n")
            file.write(f"Avg token count (right): {avg_tok_right:.2f}\n")
            file.write(f"Avg embedding count per line (left): {avg_emb_left:.2f}\n")
            file.write(f"Avg embedding count per line (right): {avg_emb_right:.2f}\n\n")

        with open(self.args.output_file, 'a') as file:
            file.write(f'Eval Dataset: {which_ds}\n\n')
            ...
            for layer in range(num_layers):
                ...
                evaluation_score = spearmanr(
                        cosine_similarities,
                        relatedness)
                #print(which_ds, add_top_blank, evaluation_score[0]) #COMMENTED OUT BY STEVEN
                file.write(f"{which_ds}, {add_top_blank}, {evaluation_score[0]}\n")
                all_layer_scores.append(evaluation_score[0]) #ADDED BY STEVEN
                layerwise_scores.append(
                        (evaluation_score[0], cosine_similarities))
            max_layer_score = max(all_layer_scores)
            print(f"max layer score for {which_ds}: {max_layer_score}") #ADDED BY STEVEN
            file.write(f"\nmax layer score for {which_ds}: {max_layer_score}\n\n\n")
            append_row_to_csv(self.args.output_csv, self.args.ckpt_path, which_ds, max_layer_score)
        
        return layerwise_scores, max_layer_score

    def do_one_eval(self, results):
        best_layer_score_lst = []
        for which_ds in self.string_lists_sims:
            if which_ds in results:
                continue
            res_w_b, max_layer_score = self.get_layerwise_scores_for_one_model_one_ds(
                    which_ds, add_top_blank=True)
            results[which_ds] = dict(
                    res_w_b=res_w_b,
                    )
            best_layer_score_lst.append(max_layer_score)
        final_model_avg_score = np.average(best_layer_score_lst)
        append_row_to_final_avg_csv(self.args.output_final_avg_score_csv, self.args.ckpt_path, final_model_avg_score)
        return results
...
def main():
    ...
    with open(args.output_file, 'a') as file:
        file.write(f'\n\n\n\nWord Relatedness Scores for: {args.ckpt_path}\n\n')
    check_and_create_csv(args.output_csv)
    check_and_create_final_avg_csv(args.output_final_avg_score_csv)
